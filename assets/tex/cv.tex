...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\usepackage[style=authoryear,backend=biber,maxnames=99,minnames=1]{biblatex}
\usepackage{etoolbox}
\usepackage{xparse}

\DeclareNameFormat{author}{%
  \ifboolexpr{
    test {\ifdefequal{\family}{Smith}}
    and
    (
      test {\ifdefequal{\given}{Isaac G.}}
      or
      test {\ifdefequal{\given}{I. G.}}
    )
  }
  {\textbf{#1}\addspace #3}
  {#1\addspace #3}
...
% New custom macro
\newcommand{\isaacgsmith@listauthors}{%
  \def\do##1{%
    \ifcsdef{labelname@##1}
      {%
        \csuse{labelname@##1}%
        \ifboolexpr{
          test {\ifdefequal{\family@##1}{Smith}}
          and
          (
            test {\ifdefequal{\given@##1}{Isaac G.}}
            or
            test {\ifdefequal{\given@##1}{I. G.}}
          )
        }
          {\global\isaacgsmith@foundtrue\global\isaacgsmith@pos=\the\value{listcount}}
          {}%
      }%
      {}%
  }%
  \dolistcsloop{blx@bbx@tempa}%
}

\newtoggle{isaacgsmith@found}
\newcounter{isaacgsmith@pos}

\newbibmacro*{author/isaacsmart}{%
  \global\togglefalse{isaacgsmith@found}%
  \global\setcounter{isaacgsmith@pos}{0}%
  \let\blx@bbx@tempa\blx@bbx@tempa@list
  \isaacgsmith@listauthors
  %
  \iftoggle{isaacgsmith@found}
    {%
      \ifnum\value{listtotal}>3
        {%
          \ifnum\value{isaacgsmith@pos}=1
            {\printnames[1]{author}\addspace et\addspace al\adddot}
          \else\ifnum\value{isaacgsmith@pos}=2
            {\printnames[2]{author}\addspace et\addspace al\adddot}
          \else\ifnum\value{isaacgsmith@pos}=3
            {\printnames[3]{author}\addspace et\addspace al\adddot}
          \else
            {\printnames[2]{author}\addcomma\space\ldots\addcomma\space\printname{author}{\value{isaacgsmith@pos}}\addspace et\addspace al\adddot}
          \fi\fi\fi
        }
        {\printnames{author}}
    }
